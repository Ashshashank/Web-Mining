...
def calculate_metrics(model, features, tags):
    ...
    accuracy = accuracy_score(tags, predictions)  # model accuracy
    precision = precision_score(tags, predictions, average='weighted')  # model precision
    recall = recall_score(tags, predictions, average='weighted')  # model recall
    f1_score_value = f1_score(tags, predictions, average='weighted')  # model f1_score
    ...
    return predictions, accuracy, precision, recall, f1_score_value, cm  # Correct the variable name here
...
def classify_ng(model_name, transfer, x_train, x_test, y_train, y_test):
    ...
    predicted_test, accuracy_test, precision_test, recall_test, f1_score_vlaue, cm_test = model_test(model, x_test, y_test)
    return predicted_test, accuracy_test, precision_test, recall_test, f1_score_vlaue, cm_test
...
def populate_df(results, model_name, accuracy, precision, recall, f1, cm, flag):
    if flag == 'DTM':
        ...
        results['DTM_F1-Score'].loc[model_name] = round(f1 * 100, 1)  # Corrected to 'DTM_F1-Score'
...
def read_text_file(path):
    ...
    # Initialize an empty DataFrame for results before the loop
...
train_folder = "/Users/shashank/Documents/WebMining/HW-4-NewsG-Classification-Py/data/20Newsgroups/20news-bydate-train/"
test_folder = "/Users/shashank/Documents/WebMining/HW-4-NewsG-Classification-Py/data/20Newsgroups/20news-bydate-test/"

doc1_train_path = os.listdir(train_folder + newsgroup1)
doc1_test_path = os.listdir(test_folder + newsgroup1)
doc2_train_path = os.listdir(train_folder + newsgroup2)
doc2_test_path = os.listdir(test_folder + newsgroup2)
...
for docs in range(count):
    temp = read_text_file(train_folder + newsgroup1 + '/' + doc1_train_path[docs])
    doc1_train.append(temp)

    temp = read_text_file(test_folder + newsgroup1 + '/' + doc1_test_path[docs])
    doc1_test.append(temp)

    temp = read_text_file(train_folder + newsgroup2 + '/' + doc2_train_path[docs])
    doc2_train.append(temp)

    temp = read_text_file(test_folder + newsgroup2 + '/' + doc2_test_path[docs])
...
for model_number in all_models.keys():
    ...
    predicted_test, accuracy_test, precision_test, recall_test, f1_score_value, cm_test = classify_ng(model_name,
                                                                                               CountVectorizer(),
                                                                                               train_data,
                                                                                               test_data,
                                                                                               train_tags,
    ...
    results = populate_df(results, model_name, accuracy_test, precision_test, recall_test, f1_score_value,
                          cm_test, flag="DTM")

    print("   ======== Using DTM =======")
    print_scores(accuracy_test, precision_test, recall_test, f1_score_value, cm_test)
    predicted_test, accuracy_test, precision_test, recall_test, f1_score_value, cm_test = classify_ng(model_name,
                                                                                               TfidfVectorizer(),
                                                                                               train_data,
                                                                                               test_data,
...
results.to_csv('/Users/shashank/Documents/WebMining/HW-4-NewsG-Classification-Py/data/results.csv')
print("End")